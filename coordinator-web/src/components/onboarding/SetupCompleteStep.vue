<template>
  <div class="w-full py-4">
    ...
    <div class="max-w-md mx-auto bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden mb-10 transform transition-all hover:-translate-y-1 duration-300">
      <div class="bg-[#1B4F72] p-6 text-center relative overflow-hidden">
        ...
        <div class="relative z-10">
          ...
          <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#10B981] text-white text-xs font-bold uppercase tracking-wide shadow-sm">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
      ...
      <div class="bg-slate-50 rounded-xl p-6 border border-slate-200">
        <div class="flex items-center gap-3 mb-4">
          ...
          <h3 class="font-semibold text-slate-800">Your Phone Number</h3>
        </div>
        <div class="bg-white p-3 rounded-lg border border-slate-200 flex justify-between items-center mb-2">
          <span class="font-mono font-medium text-lg text-slate-700">
            {{ phoneNumber || '(555) 123-4567' }}
          </span>
          <button
            @click="copyPhoneNumber"
            class="text-slate-400 hover:text-[#1B4F72] transition-colors"
            title="Copy number"
          >
            <Copy :size="16" />
          </button>
      ...
      <div class="bg-slate-50 rounded-xl p-6 border border-slate-200">
        <div class="flex items-center gap-3 mb-4">
          ...
          <h3 class="font-semibold text-slate-800">First Steps</h3>
        </div>
        <ul class="space-y-3">
          <li
      ...
      <div class="bg-slate-50 rounded-xl p-6 border border-slate-200">
        <div class="flex items-center gap-3 mb-4">
          ...
          <h3 class="font-semibold text-slate-800">Your Trial</h3>
        </div>
        <div class="mb-4">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
      <router-link
        to="/dashboard"
        class="w-full sm:w-auto px-8 py-3.5 rounded-lg font-semibold text-white bg-[#1B4F72] hover:bg-[#153e5a] hover:-translate-y-0.5 transition-all duration-200 shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
      >
        Go to Dashboard
        <ArrowRight :size="18" />
      </router-link>
      <button class="text-slate-500 hover:text-[#1B4F72] font-medium text-sm transition-colors">

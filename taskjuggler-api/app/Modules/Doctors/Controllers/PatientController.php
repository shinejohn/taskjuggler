...
{
    public function __construct(private \App\Modules\Doctors\Services\ConsentService $consentService) {}
    ...
    {
        $patient = DB::table('doctors_patients')->where('id', $id)->first();
        if (!$patient) {
            return response()->json(['message' => 'Patient not found'], 404);
        }

        // Local Encounters
        $encounters = DB::table('doctors_clinical_encounters')
            ->where('patient_id', $id)
            ->orderBy('encounter_date', 'desc')
            ->get();

        // Federated (Network) History
        $networkHistory = $this->consentService->getFederatedHistory($id);
    ...
    /**
     * Lookup patient by Universal Health ID (Sharing Code)
     */
    public function lookupByCode(Request $request)
    {
        $request->validate([
            'sharing_code' => 'required|string'
        ]);

        $patient = DB::table('doctors_patients')
            ->where('universal_health_id', $request->sharing_code)
            ->first();

        if (!$patient) {
            return response()->json([
                'success' => false,
                'message' => 'No patient found with this Health ID.'
            ], 404);
        }

        // Return limited PDI for confirmation
        return response()->json([
            'success' => true,
            'patient' => [
                'id' => $patient->id,
                ...
                'dob' => $patient->dob, // mask this in production
            ]
        ]);
    }
}

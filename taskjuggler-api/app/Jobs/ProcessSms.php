...
class ProcessSms implements ShouldQueue
{
    ...
    public function handle(
        ...
        $inboxItem = InboxItem::create([
            ...
            'source_type' => InboxItem::SOURCE_SMS,
            'source_id' => $this->messageSid,
        ...
        try {
            // 2. AI extraction
            $extracted = $extractor->extract($this->body, 'sms');
            $extracted['source'] = 'sms';
            $extracted['from_number'] = $this->fromNumber;
            $extracted['is_command'] = $this->isCommand;

            // Use phone number as contact if not extracted
            if (empty($extracted['contact']['phone'])) {
                $extracted['contact']['phone'] = $this->fromNumber;
            }
            ...
            $task = Task::create([
                ...
                'tags' => $decision->tags,
                'metadata' => ['message_sid' => $this->messageSid],
            ]);
